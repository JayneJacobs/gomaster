...
func main() {
	db, err := sql.Open("mysql", "hydra:hydraisme@/Hydra?parseTime=true")
	if err != nil {
		log.Fatal("Could not connect, error ", err.Error())
	}
	defer db.Close()

	cw := GetCrewByPositions(db, []string{"'Drums'", "'Flute'", "'Chemist'"}) //Needs single quotations
	fmt.Println(cw)

	fmt.Println(GetCrewMemberById(db, 1))

	AddCrewMember(db, crewMember{name: "Kaya Gal", secClearance: 4, position: "Biologist"})

	fmt.Println(GetCrewMemberByPosition(db, "Guitar"))
...
// GetCrewByPositions takes the db conn string and positions as a string array. It returns a Crew type
func GetCrewByPositions(db *sql.DB, positions []string) Crew {

	//use strings.Join(stringArray, string)
	Qs := fmt.Sprintf("SELECT id,Name,SecurityClearance,Position from Personnel where Position in (%s);", strings.Join(positions, ","))
	//Should look like -- SELECT id,Name,SecurityClearance,Position from Personnel where Position in (mechanic,biologist)
